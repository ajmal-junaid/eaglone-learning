  ...
  addToCart: async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ err: true, message: 'Invalid product ID or user ID' })
      }
      ...
      if (user.coursesPurchased.includes(courseId)) {
        return res
          .status(400)
          .json({ err: true, message: 'Course already purchased' })
      }
      ...
      if (result.modifiedCount === 0) {
        res
          .status(400)
          .json({ err: true, message: 'Course Already Exists in Cart' })
      } else {
  ...
  addFreeCourse: async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ err: true, message: 'Invalid course ID or user ID' })
      }
      const result = await User.updateOne(
      ...
      if (result.modifiedCount === 0) {
        ...
        res
          .status(200)
          .json({ err: false, message: 'Course Added Successfull' })
      }
    } catch (error) {
      console.log(error)
      res.status(500).json({ err: true, message: 'Something Went Wrong' })
  ...
  removeCourse: async (req, res) => {
    try {
      ...
      if (!result.cart.includes(courseId)) {
        return res
          .status(400)
          .json({ err: true, message: 'Product Not Found in Cart' })
      }
  ...
  getCartCourses: async (req, res) => {
    try {
      ...
      if (!mongoose.isValidObjectId(userId)) {
        return res
          .status(400)
          .json({ err: true, message: 'Invalid user ID', data: [] })
      }
      ...
      if (!user) {
        return res
          .status(404)
          .json({ err: true, message: 'User Not Found', data: [] })
      }
      const cart = user.cart
      if (!cart.length > 0)
        return res
          .status(404)
          .json({ err: true, message: 'Cart is empty', data: [] })
      ...
      res
        .status(500)
        .json({ err: true, message: 'Something Went Wrong', data: [] })
    }
  },
}
