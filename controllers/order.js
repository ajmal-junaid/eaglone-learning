...
module.exports = {
  createOrder: async (req, res) => {
    try {
      ...
      const userCoursesPurchased = await User.findById(user).select(
        'coursesPurchased'
      )
      const purchasedCourseIds = userCoursesPurchased.coursesPurchased.map(
        (course) => course.toString()
      )
      if (purchasedCourseIds.length) {
        const coursesToPurchase = courses.filter(
          (course) => !purchasedCourseIds.includes(course.toString())
        )
        if (coursesToPurchase.length === 0) {
          return res.status(400).json({
            err: true,
            message: 'User has already purchased All these courses',
          })
        } else {
          return res.status(400).json({
            err: true,
            message: 'Already purchased some of these courses',
          })
        }
      }
      ...
      if (!courseDocuments)
        return res.status(400).json({
          err: true,
          message: 'Courses is removed or expired (not found)',
        })
      if (paymentStatus)
        await User.findByIdAndUpdate(
          user,
          { $addToSet: { coursesPurchased: { $each: courses } } },
          { new: true }
        )
      Order.create({
        user: user,
        coupon: coupon,
        client: client,
        courses: courseDocuments.map((course) => ({
          course: course._id,
        ...
        .catch((err) => {
          res
            .status(201)
            .json({ err: true, message: 'order failed', data: err })
        })
    } catch (err) {
      console.error(err)
  ...
  verifyPayment: async (req, res) => {
    try {
      ...
      res.status(500).json({
        err: true,
        message: 'payment failed , if money debited, contact us',
        error,
      })
    }
  },
  getOrders: async (req, res) => {
    try {
      ...
      if (!orders)
        return res
          .status(404)
          .json({ err: true, message: 'no orders found on this user' })
      return res
        .status(200)
        .json({ err: false, message: 'orders fetched', data: orders })
    } catch (error) {
      res
        .status(500)
  ...
  getAllOrders: async (req, res) => {
    try {
      ...
      if (!orders)
        return res
          .status(404)
          .json({ err: true, message: 'no orders found in Db' })
      return res
        .status(200)
        .json({ err: false, message: 'orders fetched', data: orders })
    } catch (error) {
