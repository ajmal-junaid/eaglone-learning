...
module.exports = {
  ...
  addCourse: async (req, res) => {
    try {
      ...
      if (!courseId || !title || !category || !premium)
        return res
          .status(422)
      ...
      if (course)
        return res.status(409).json({
          err: true,
          message: 'Course with this Id is Already Exists',
        })
      ...
      req.body.ourPrice = Math.round(
        req.body.price - req.body.price * (req.body.percentage / 100)
      )
      ...
      return res
        .status(500)
        .json({ err: true, message: 'Course Creation Failed' })
    } catch (error) {
      return res
        .status(500)
  ...
  getCourseById: async (req, res) => {
    try {
      ...
      if (!course)
        return res.status(204).json({ err: true, message: 'No course found' })
      return res
        .status(200)
  ...
  getCourseByCategoryName: async (req, res) => {
    try {
      ...
      if (courses.length < 1)
        return res
          .status(404)
          .json({ err: true, message: 'No courses found under this category' })
      return res
        .status(200)
  ...
  searchCourse: async (req, res) => {
    try {
      ...
      if (!result.length)
        return res
          .status(404)
          .json({ err: true, message: 'course not found', data: null })
      return res.status(200).json({
        err: false,
        message: 'courses fetched successfully',
        data: result,
  ...
  rateCourse: async (req, res) => {
    try {
      ...
      if (isRatingExists)
        return res
          .status(400)
          .json({ err: true, message: 'you have already rated this course' })
