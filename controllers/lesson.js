...
module.exports = {
  ...
  addLesson: async (req, res) => {
    try {
      ...
      if (!lessonId || !title || !tutorName || !course)
        return res
          .status(422)
      ...
      if (lesson)
        return res.status(409).json({
          err: true,
          message: 'lesson with this id is already exists',
        })
      ...
      if (success)
        ...
        return res.status(500).json({ err: true, message: 'lesson not added' })
    } catch (error) {
      console.log(error)
      return res
        .status(500)
        .json({ err: true, message: 'operation failed ', reason: error })
    }
  },
    ...
    try {
      ...
      if (!lessons)
        return res.status(404).json({
          err: true,
          message: 'lessons not found under this category',
        })
      return res
        .status(200)
        .json({ message: 'lessons fetched succesfully', data: lessons })
    } catch (error) {
      return res
        .status(500)
        .json({ err: true, message: 'operation failed ', reason: error })
  ...
  getALesson: async (req, res) => {
    try {
      ...
      if (!lesson)
        return res
          .status(404)
          .json({ err: true, message: 'lesson not found under this category' })
      return res
        .status(200)
        .json({ message: 'lesson fetched succesfully', data: lesson })
    } catch (error) {
      return res
        .status(500)
        .json({ err: true, message: 'operation failed ', reason: error })
  ...
  deleteLesson: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: 'Lesson deleted Successfully', data: result })
    } catch (error) {
      return res
        .status(500)
  ...
  updateLesson: async (req, res) => {
    try {
      const lesson = await Lesson.findOne({ _id: req.params.id })
      if (!lesson)
        return res.status(404).json({ err: true, message: 'No Lesson found' })
      const videoUrl = req.file
        ? `https://${process.env.AWS_S3_BUCKET_NAME}.s3.amazonaws.com/${req.file.key}`
        : lesson.video
      req.body.video = videoUrl
  ...
  getAllLessonsByCourse: async (req, res) => {
    try {
      const lessons = await Lesson.find({ course: req.params.id }).select(
        '-video'
      )
      if (!lessons)
        return res
          .status(404)
          .json({ err: true, message: 'lessons not found under this course' })
      return res
        .status(200)
        .json({ message: 'lessons fetched succesfully', data: lessons })
    } catch (error) {
  ...
  getAllLessonsByCourseId: async (req, res) => {
    try {
      ...
      if (!lessons.length)
        return res
          .status(404)
          .json({ err: true, message: 'no lesson found under this course' })
      return res
        .status(200)
        .json({ message: 'lessons fetched succesfully', data: lessons })
    } catch (error) {
